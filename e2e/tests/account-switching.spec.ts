...
test.describe("Account Switching", () => {
  ...
  test("can open and close link account modal from profile", async ({
    ...
    // Close via the close button inside the modal (scoped to avoid matching toast close button)
    await page.getByTestId("link-account-modal").getByRole("button", { name: "Close" }).click();
