...
import { respondToFriendRequestByActor } from "@/app/feed/friend-actions";
...
export function FriendRequestNotificationActions({
  actorId,
  onRespond,
}: FriendRequestNotificationActionsProps) {
  ...
  function handleAction(action: "accept" | "decline") {
    startTransition(async () => {
      const result = await respondToFriendRequestByActor(actorId, action);
      if (result.success) {
