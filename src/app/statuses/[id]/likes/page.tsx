...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  return {
    title: status?.user ? `Likes on @${status.user.username}'s status` : "Status Likes",
...
export default async function StatusLikesPage({ params }: Props) {
  ...
  const status = await prisma.userStatus.findUnique({
    ...
    select: {
      id: true,
      content: true,
      user: { select: { ...USER_SELECT, ...userThemeSelect } },
    },
  });

  if (!status?.user) notFound();
  ...
  const followingSet = new Set(followData.map((f: { followingId: string }) => f.followingId));
  for (const user of users) {
    user.isFollowing = followingSet.has(user.id);
  }

  const theme = buildUserTheme(status.user);
