...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  return {
    title: post?.author ? `Reposts of @${post.author.username}'s post` : "Post Reposts",
...
export default async function PostRepostsPage({ params }: Props) {
  ...
  // Deduplicate users — a user could have both a direct repost and a quote repost
  const seenIds = new Set<string>();
  const uniqueUsers: FollowUser[] = [];
  for (const repost of reposts) {
    if (!seenIds.has(repost.user.id)) {
      seenIds.add(repost.user.id);
      uniqueUsers.push({ ...repost.user, isFollowing: false });
    }
  }

  const directCount = reposts.filter((r) => !r.quotedRepostId && !r.content).length;
  const quoteCount = reposts.filter((r) => r.content || r.quotedRepostId).length;

  const otherUserIds = uniqueUsers.filter((u) => u.id !== currentUserId).map((u) => u.id);
      ...
      <div className="rounded-2xl bg-white shadow-lg dark:bg-zinc-900">
        <div className="flex items-center gap-3 border-b border-zinc-100 px-4 py-3 dark:border-zinc-800">
          ...
          <h1 className="text-lg font-semibold text-zinc-900 dark:text-zinc-100">
            Reposted by ({uniqueUsers.length})
          </h1>
          {(directCount > 0 || quoteCount > 0) && (
            <div className="ml-auto flex gap-3 text-xs text-zinc-500 dark:text-zinc-400">
              {directCount > 0 && <span>{directCount} repost{directCount !== 1 ? "s" : ""}</span>}
